...
CREATE INDEX IF NOT EXISTS `idx_corpus_category_merchant` ON `corpus` (`category_id`, `merchant_id`);
...
-- ============================================
-- 第五部分：Mapper 方法对应的 SQL 语句
-- ============================================

-- order_record: 删除过期订单（需要在 Mapper 中实现）
...
-- UPDATE coupon
-- SET status = '2' (已过期)
-- WHERE expired_at IS NOT NULL AND expired_at < NOW() AND status != '2';
