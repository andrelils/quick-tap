...
function queue$2(hooks, data, params2) {
  let promise2 = false;
...
function hasCallback(args) {
  if (isPlainObject$3(args) && [API_SUCCESS, API_FAIL, API_COMPLETE].find((cb) => isFunction(args[cb]))) {
    return true;
  }
  return false;
}
...
class Router {
  ...
  async route(options = {}, params2 = {}) {
    ...
    if (mergeConfig2.url === page()) return;
...
function getRuntimeU(upU) {
  if (upU) return upU;
  if (typeof index$1 !== "undefined") return index$1.$u;
  return null;
}
function normalizeRuntimeRoute(route2) {
  if (typeof route2 !== "string") return "";
...
function getCurrentRuntimeRoute() {
  try {
    ...
    const pages2 = getCurrentPages();
    if (!Array.isArray(pages2) || pages2.length === 0) return "";
...
function trySetNavigationBarColor$1(options) {
  if (typeof index$1 === "undefined" || typeof index$1.setNavigationBarColor !== "function") return;
  if (!hasActiveRuntimePage$1()) return;
  try {
...
function trySetTabBarStyle(options) {
  if (typeof index$1 === "undefined" || typeof index$1.setTabBarStyle !== "function") return;
...
function syncThemeRuntimeFromStorage(upU) {
  ...
  if (!runtimeU || !runtimeU.theme) return runtimeU == null ? void 0 : runtimeU.theme;
  const preference = getFallbackThemePreference();
...
function applyNativeThemeUI$1(upU) {
  ...
  if (typeof index$1 === "undefined") return;
  const runtimeU = getRuntimeU(upU);
  if (((_a = runtimeU == null ? void 0 : runtimeU.config) == null ? void 0 : _a.nativeThemeSync) !== true) return;
  const isDark = getThemeIsDark(runtimeU);
...
const defProps$1 = registerComponentProps(IconDefaultProps);
const props$1 = defineMixin({
  props: {
    // 图标类名
    name: {
      type: String,
      default: () => defProps$1.icon.name
    },
    // 图标颜色，可接受主题色
    color: {
      type: String,
      default: () => defProps$1.icon.color
    },
    // 字体大小，单位px
    size: {
      type: [String, Number],
      default: () => defProps$1.icon.size
    },
    // 是否显示粗体
    bold: {
      type: Boolean,
      default: () => defProps$1.icon.bold
    },
    // 点击图标的时候传递事件出去的index（用于区分点击了哪一个）
    index: {
      type: [String, Number],
      default: () => defProps$1.icon.index
    },
    // 触摸图标时的类名
    hoverClass: {
      type: String,
      default: () => defProps$1.icon.hoverClass
    },
    // 自定义扩展前缀，方便用户扩展自己的图标库
    customPrefix: {
      type: String,
      default: () => defProps$1.icon.customPrefix
    },
    // 图标右边或者下面的文字
    label: {
      type: [String, Number],
      default: () => defProps$1.icon.label
    },
    // label的位置，只能右边或者下边
    labelPos: {
      type: String,
      default: () => defProps$1.icon.labelPos
    },
    // label的大小
    labelSize: {
      type: [String, Number],
      default: () => defProps$1.icon.labelSize
    },
    // label的颜色
    labelColor: {
      type: String,
      default: () => defProps$1.icon.labelColor
    },
    // label与图标的距离
    space: {
      type: [String, Number],
      default: () => defProps$1.icon.space
    },
    // 图片的mode
    imgMode: {
      type: String,
      default: () => defProps$1.icon.imgMode
    },
    // 用于显示图片小图标时，图片的宽度
    width: {
      type: [String, Number],
      default: () => defProps$1.icon.width
    },
    // 用于显示图片小图标时，图片的高度
    height: {
      type: [String, Number],
      default: () => defProps$1.icon.height
    },
    // 用于解决某些情况下，让图标垂直居中的用途
    top: {
      type: [String, Number],
      default: () => defProps$1.icon.top
    },
    // 是否阻止事件传播
    stop: {
      type: Boolean,
      default: () => defProps$1.icon.stop
    }
  }
});
